...
void ImGuiOverlayDrawList::add_line(OverlayPoint p0, OverlayPoint p1,
...
void ImGuiOverlayDrawList::add_rect_filled(OverlayPoint p0, OverlayPoint p1,
                                           OverlayColor color, float rounding,
                                           OverlayRoundCorners corners)
{
    if (dl_)
        dl_->AddRectFilled(to_imvec2(p0), to_imvec2(p1), to_imu32(color),
                           rounding, to_imdraw_flags(corners));
}

void ImGuiOverlayDrawList::add_rect(OverlayPoint p0, OverlayPoint p1,
                                    OverlayColor color, float rounding,
                                    OverlayRoundCorners corners, float thickness)
{
    if (dl_)
        dl_->AddRect(to_imvec2(p0), to_imvec2(p1), to_imu32(color),
                     rounding, to_imdraw_flags(corners), thickness);
}

void ImGuiOverlayDrawList::add_circle_filled(OverlayPoint center, float radius,
                                             OverlayColor color, int num_segments)
{
    if (dl_)
        dl_->AddCircleFilled(to_imvec2(center), radius, to_imu32(color), num_segments);
}

void ImGuiOverlayDrawList::add_circle(OverlayPoint center, float radius,
                                      OverlayColor color, int num_segments,
                                      float thickness)
{
    if (dl_)
...
void ImGuiOverlayDrawList::add_triangle_filled(OverlayPoint p0, OverlayPoint p1,
...
void ImGuiOverlayDrawList::add_text(OverlayFont* font, float font_size,
...
OverlayPoint ImGuiOverlayDrawList::text_size(OverlayFont* font, float font_size,
